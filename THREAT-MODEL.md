...
| **T6** | DoS | `RetryPolicy` retried *all* exceptions, amplifying load on non-transient failures. | Low | **Fixed** — `ValueError`/`TypeError` added to the no-retry skip list; guidance to narrow `retry_on` for provider 4xx/auth. |
