...
What `marrow` is working toward, in priority order. Dates are intent; OSS schedules are aspirational.
...
- [x] Structured logging via `marrow.logging_config`
...
- [ ] Provider plugin system — third-party providers as `marrow-provider-*` packages
...
- [ ] C++ shared library build (`libmarrow.so` / `.dylib` / `.dll`)
